 ...
 *
 * The STRONGEST check: a litesvm test that creates a REAL `Protocol` account via
 * the program's `init_protocol`, fetches the raw bytes, and decodes them with
 * the SDK — proving the offsets against the real program, not just the synthetic
 * buffers.
 */
...
import { AccountType, ACCOUNT_SIZES, Ix, Phase } from "../src/constants.js";
import {
  decodeAiClaim,
  decodeFact,
  decodeFactVote,
...
function key32Addr(n: number): string {
