 ...
 *
 * This is the TypeScript mirror of `kassandra-market/sdk-rs/src/metadao.rs`, the
...
/** MetaDAO `conditional_vault` v0.4.0 program id (`sdk-rs` `CONDITIONAL_VAULT_ID`). */
export const CONDITIONAL_VAULT_ID = EXTERNAL_PROGRAM_IDS.conditionalVault;
/** MetaDAO `amm` v0.4.2 delayed-twap program id (`sdk-rs` `AMM_ID`). */
export const AMM_V04_ID = EXTERNAL_PROGRAM_IDS.ammV04;
/** SPL Associated-Token-Account program (`sdk-rs` `ASSOCIATED_TOKEN_PROGRAM_ID`). */
 ...
 * byte for byte from `sdk-rs/src/metadao.rs`.
 *
 * `crankThatTwap` is NOT declared in `sdk-rs` (the Rust side never composes it);
 * its value is the binary-validated one from the sibling AMM SDK.
 */
export const DISC = {
  // conditional_vault
  /** `conditional_vault::initialize_question` (`sdk-rs` `INITIALIZE_QUESTION_DISC`). */
  ...
  // amm v0.4
  /** `amm::create_amm` (`sdk-rs` `CREATE_AMM_DISC`). */
  ...
  /** `amm::crank_that_twap` — not in `sdk-rs`; sibling binary-validated value. */
  crankThatTwap: d([0xdc, 0x64, 0x19, 0xf9, 0x00, 0x5c, 0xc3, 0xc1]),
} as const;

/** Anchor `Amm` account discriminator — `sha256("account:Amm")[..8]` (`sdk-rs` `AMM_ACCOUNT_DISCRIMINATOR`). */
...
/** PDA seed-prefix byte strings (`sdk-rs/src/metadao.rs`). */
export const SEED = {
  /** `Question` PDA prefix. */
  question: e("question"),
