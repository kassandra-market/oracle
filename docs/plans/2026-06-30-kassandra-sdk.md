...
### D5 — packaging + docs + example (DONE 2026-06-30)

**Files added:** `sdk/src/index.ts` (the public barrel), `sdk/tsconfig.build.json` (the emit config), `sdk/README.md`, `sdk/examples/quickstart.ts`. **Edited:** `sdk/package.json` (build script + `main`/`module`/`types`/`exports`/`files`/`sideEffects`), `sdk/tsconfig.json` (added `examples` to `include` so the example typechecks).

**Public API surface (`src/index.ts`):** one barrel re-exporting `export *` from constants, pda (flat AND a `pda` namespace — so both `oracle(1n)` and `pda.oracle(1n)` work), accounts/index, instructions/index, and litesvm-interop. So consumers get: the constants (`KASSANDRA_PROGRAM_ID`/`SYSTEM_PROGRAM_ID`/`TOKEN_PROGRAM_ID`, `Ix`, `AccountType`, `Phase`, `KassandraError`+`decodeError`, `ACCOUNT_SIZES`, `EXTERNAL_PROGRAM_IDS`, `CONFIG`, the `CLAIM_OPTION_NONE`/`VOTE_*` sentinels); all 10 PDA fns + `Pda`/`AddressInput`; all 7 `decodeX` + their types + the common readers; all 22 builders + their `*Args` types + `encodeSetConfigParams` + the payload helpers; and `toLiteSvmTransaction`. No `export *` name collisions (pda fns are lowercase `oracle`/`fact`/`market`; account types are PascalCase `Oracle`/`Fact`/`Market` + `decodeOracle` — distinct identifiers). The litesvm bridge is kept IN the main barrel (not a separate subpath) because `litesvm-interop.ts` imports only a **type** from litesvm + `getTransactionDecoder` from kit — it pulls in NO litesvm runtime, so it is safe to import without litesvm installed.

**Build tool — plain `tsc` (no new dep).** The package was already ESM (`"type":"module"`) with `.js`-extension relative imports, so `tsc -p tsconfig.build.json` emits a clean dual ESM `.js` + `.d.ts` (+ declaration/source maps) to `dist/` with zero added tooling (chose tsc over tsup on the YAGNI principle — tsup would add a dependency for no gain here). `tsconfig.build.json` extends the base, flips `noEmit:false`, sets `outDir:dist`/`rootDir:src`, `include:["src"]` (tests/examples excluded from the artifact). `package.json` `exports`/`main`/`module` → `./dist/index.js`, `types` → `./dist/index.d.ts`, `files:["dist","README.md"]`, `sideEffects:false`. `dist/` is gitignored (D0) — NOT committed.

**README sections:** program id + client-lib note (web3.js v3 classic, async sign/PDA); install (pnpm/npm); build/typecheck/test; the **`just build` → `.so` prerequisite** for litesvm usage/tests; a **quickstart** (derive `pda.oracle(1n)` → build `createOracle` → sign/send with web3.js v3 **noting `sign()`/`serialize()` are async** → litesvm bridge via `toLiteSvmTransaction` → `decodeOracle` on fetched bytes); tables of the 22 builders + 7 decoders; `decodeError`; constants; a **Known limitations** section (see below); and a layout map.

**The v0-tx caveat (called out in the README "Known limitations"):** the SDK emits **legacy** transactions; `finalizeProposals`/`finalizeOracle` append the FULL proposer set as an account tail, so a near-cap set (`MAX_PROPOSERS == 60`) exceeds a legacy tx's 1232-byte packet and would need a **versioned (v0) tx + Address Lookup Table** to fit — documented as a known limitation / future work. Also documented: emissions are governance-enabled + default-disabled (`emission_num == 0` at genesis), and MetaDAO market accounts are caller-supplied.

**Example (`examples/quickstart.ts`):** the example-shaped twin of `test/e2e.test.ts` — derive PDAs → `initProtocol` → `createOracle` → `propose`×3 → `finalizeProposals` → decode the Oracle (asserts `Resolved`/`resolvedOption`), with `console.log` output and comments, reusing the litesvm SPL-fabrication + clock-warp setup. Imports exclusively from the public barrel (`../src/index.js`), is typechecked by `pnpm typecheck`, and is genuinely **runnable** — verified end-to-end against the real `.so` via `pnpm dlx tsx examples/quickstart.ts` (printed `oracle.phase === Resolved: true`, `resolvedOption: 1`). The header marks the `just build` requirement to run it.

**Green:** `pnpm build` → clean `dist/` with `.d.ts` + ESM `.js`; `pnpm typecheck` clean (src + test + example); `pnpm test` = **65 passed** (unchanged). No source/decoder/builder changes — D5 is packaging + docs + example only.

### SDK: covered vs deferred

**Covered (D0–D5):** all 22 instruction builders; all 7 Pod account decoders + type-tag/size validation; all 10 PDA derivations; constants/errors (`decodeError`); the web3.js-v3 ↔ litesvm bridge; a parity drift-guard (discriminants/sizes vs the pinned Rust values); a real-account decode (`init_protocol`); a litesvm END-TO-END lifecycle through the SDK against the real `.so` (init → create → propose → finalize → Resolved, + a FactProposal dispute slice); packaging (`dist/` ESM + types via tsc), README, and a runnable example. 65 tests green.

**Deferred (later):**
- **Widen live (litesvm) coverage to the dispute/challenge/settlement builders** (final-review follow-up). The litesvm E2E (D4) drives the lifecycle quartet (initProtocol/createOracle/propose/finalizeProposals) against the real `.so`; the other 18 builders are covered by unit byte-assertions mirrored from the processors + harness `*_ix` (consistent with the SDK's stated byte-mirroring contract). An off-by-one mirrored identically in both builder and test would not be caught live — so drive each remaining builder (esp. the 25/21-account openChallenge/settleChallenge + the finalize tails + claims/closes) through a real litesvm transaction to prove account orders by execution, not only by assertion.
- **Versioned (v0) tx + Address Lookup Table** for large `finalizeProposals`/`finalizeOracle` (near-cap proposer sets overflow a legacy tx's 1232-byte packet) — the SDK currently emits legacy txs only.
- **npm publish** — package stays `private`/in-repo (`dist/` consumed locally); the `main`/`module`/`types`/`exports`/`files` fields are in place for when publishing is in scope.
- **MetaDAO-market composition helpers** for `openChallenge`/`settleChallenge` — those market/AMM/conditional-vault/mint accounts are currently **caller-supplied** (the SDK derives only the Kassandra-owned PDAs); composing the MetaDAO market end-to-end is out of scope (covered by the Rust suite).
- Per the plan's original out-of-scope: the off-chain AI runner, the app/frontend, surfpool/real-validator E2E, and auto-generating the SDK (this is the hand-written client by design).
